hoistPattern:
  - '*'
hoistedDependencies:
  /@babel/runtime/7.21.0:
    '@babel/runtime': private
  /@onflow/config/1.0.5:
    '@onflow/config': private
  /@onflow/interaction/0.0.11:
    '@onflow/interaction': private
  /@onflow/rlp/1.0.4:
    '@onflow/rlp': private
  /@onflow/sdk/1.1.2:
    '@onflow/sdk': private
  /@onflow/transport-http/1.5.0:
    '@onflow/transport-http': private
  /@onflow/types/1.0.5:
    '@onflow/types': private
  /@onflow/util-actor/1.1.2:
    '@onflow/util-actor': private
  /@onflow/util-address/1.0.3:
    '@onflow/util-address': private
  /@onflow/util-invariant/1.0.3:
    '@onflow/util-invariant': private
  /@onflow/util-logger/1.1.2:
    '@onflow/util-logger': private
  /@onflow/util-template/1.0.4:
    '@onflow/util-template': private
  /@onflow/util-uid/1.0.3:
    '@onflow/util-uid': private
  /@types/node/18.14.6:
    '@types/node': private
  /@types/webidl-conversions/7.0.0:
    '@types/webidl-conversions': private
  /@types/whatwg-url/8.2.2:
    '@types/whatwg-url': private
  /accepts/1.3.8:
    accepts: private
  /array-flatten/1.1.1:
    array-flatten: private
  /asynckit/0.4.0:
    asynckit: private
  /base64-js/1.5.1:
    base64-js: private
  /bson/5.0.1:
    bson: private
  /buffer/6.0.3:
    buffer: private
  /bytes/3.1.2:
    bytes: private
  /call-bind/1.0.2:
    call-bind: private
  /combined-stream/1.0.8:
    combined-stream: private
  /content-disposition/0.5.4:
    content-disposition: private
  /content-type/1.0.5:
    content-type: private
  /cookie-signature/1.0.6:
    cookie-signature: private
  /cookie/0.5.0:
    cookie: private
  /debug/2.6.9:
    debug: private
  /deepmerge/4.3.0:
    deepmerge: private
  /delayed-stream/1.0.0:
    delayed-stream: private
  /depd/2.0.0:
    depd: private
  /destroy/1.2.0:
    destroy: private
  /ee-first/1.1.1:
    ee-first: private
  /encodeurl/1.0.2:
    encodeurl: private
  /escape-html/1.0.3:
    escape-html: private
  /etag/1.8.1:
    etag: private
  /finalhandler/1.2.0:
    finalhandler: private
  /follow-redirects/1.15.2:
    follow-redirects: private
  /form-data/4.0.0:
    form-data: private
  /forwarded/0.2.0:
    forwarded: private
  /fresh/0.5.2:
    fresh: private
  /function-bind/1.1.1:
    function-bind: private
  /get-intrinsic/1.2.0:
    get-intrinsic: private
  /has-symbols/1.0.3:
    has-symbols: private
  /has/1.0.3:
    has: private
  /http-errors/2.0.0:
    http-errors: private
  /iconv-lite/0.4.24:
    iconv-lite: private
  /ieee754/1.2.1:
    ieee754: private
  /inherits/2.0.4:
    inherits: private
  /ip/2.0.0:
    ip: private
  /ipaddr.js/1.9.1:
    ipaddr.js: private
  /kareem/2.5.1:
    kareem: private
  /media-typer/0.3.0:
    media-typer: private
  /memory-pager/1.5.0:
    memory-pager: private
  /merge-descriptors/1.0.1:
    merge-descriptors: private
  /methods/1.1.2:
    methods: private
  /mime-db/1.52.0:
    mime-db: private
  /mime-types/2.1.35:
    mime-types: private
  /mime/1.6.0:
    mime: private
  /mongodb-connection-string-url/2.6.0:
    mongodb-connection-string-url: private
  /mongodb/5.1.0:
    mongodb: private
  /mpath/0.9.0:
    mpath: private
  /mquery/5.0.0:
    mquery: private
  /ms/2.1.3:
    ms: private
  /negotiator/0.6.3:
    negotiator: private
  /node-bin-setup/1.1.3:
    node-bin-setup: private
  /node-fetch/2.6.9:
    node-fetch: private
  /object-assign/4.1.1:
    object-assign: private
  /object-inspect/1.12.3:
    object-inspect: private
  /on-finished/2.4.1:
    on-finished: private
  /parseurl/1.3.3:
    parseurl: private
  /path-to-regexp/0.1.7:
    path-to-regexp: private
  /proxy-addr/2.0.7:
    proxy-addr: private
  /proxy-from-env/1.1.0:
    proxy-from-env: private
  /punycode/2.3.0:
    punycode: private
  /qs/6.11.0:
    qs: private
  /queue-microtask/1.1.2:
    queue-microtask: private
  /range-parser/1.2.1:
    range-parser: private
  /raw-body/2.5.2:
    raw-body: private
  /regenerator-runtime/0.13.11:
    regenerator-runtime: private
  /safe-buffer/5.2.1:
    safe-buffer: private
  /safer-buffer/2.1.2:
    safer-buffer: private
  /saslprep/1.0.3:
    saslprep: private
  /send/0.18.0:
    send: private
  /serve-static/1.15.0:
    serve-static: private
  /setprototypeof/1.2.0:
    setprototypeof: private
  /sha3/2.1.4:
    sha3: private
  /side-channel/1.0.4:
    side-channel: private
  /sift/16.0.1:
    sift: private
  /smart-buffer/4.2.0:
    smart-buffer: private
  /socks/2.7.1:
    socks: private
  /sparse-bitfield/3.0.3:
    sparse-bitfield: private
  /statuses/2.0.1:
    statuses: private
  /toidentifier/1.0.1:
    toidentifier: private
  /tr46/0.0.3:
    tr46: private
  /type-is/1.6.18:
    type-is: private
  /unpipe/1.0.0:
    unpipe: private
  /utils-merge/1.0.1:
    utils-merge: private
  /vary/1.1.2:
    vary: private
  /webidl-conversions/3.0.1:
    webidl-conversions: private
  /whatwg-url/5.0.0:
    whatwg-url: private
included:
  dependencies: true
  devDependencies: true
  optionalDependencies: true
injectedDeps: {}
layoutVersion: 5
nodeLinker: isolated
packageManager: pnpm@7.27.0
pendingBuilds: []
prunedAt: Thu, 09 Mar 2023 07:48:41 GMT
publicHoistPattern:
  - '*eslint*'
  - '*prettier*'
registries:
  default: https://registry.npmjs.org/
skipped: []
storeDir: /Users/thanghoduc/Library/pnpm/store/v3
virtualStoreDir: .pnpm
